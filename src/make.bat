...
..\bin\mkisofs.exe -quiet -abstract ABS.TXT -biblio BIB.TXT -copyright CPY.TXT -A "DEVON" -V "BAD_APPLE" -publisher "DEVON" -p "DEVON" -sysid "MEGA_CD" -iso-level 1 -o files.bin -pad _files

..\bin\asm68k.exe /p /q cdip\ip.asm, cdip\ip.bin,,cdip\ip.lst
..\bin\asm68k.exe /p /q cdsp\sp.asm, cdsp\sp.bin,,cdsp\sp.lst
..\bin\asm68k.exe /p /q main.asm, _out\badapple.iso
del files.bin > nul
